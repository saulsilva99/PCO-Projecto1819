package fcul.pco.eurosplit.main;

/**
 *
 * @author tl
 */
public class ApplicationConfiguration {

    /**
     * The directory where files are saved.
     */
    public static final String ROOT_DIRECTORY = "./users.dat";
    public static final String ROOT_DIRECTORY_Split = "./splits.dat";
    public static final String ROOT_DIRECTORY_Expenses = "./expenses.dat";
    public static String USER_CATALOG_FILENAME = "userCatalog";
    public static String SPLIT_CATALOG_FILENAME = "splitCatalog";
    public static String EXPENSES_CATALOG_FILENAME = "expensesCatalog";

    static String DEFAULT_PROMPT = "EuroSplit";  


}